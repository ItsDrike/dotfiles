...
Once finished, this method will basically store another decryption key to your
encrypted partition in the TPM module. During boot, the initrd will request
this decryption key from TPM, which will only release it under certain
...
[SECURE_BOOT](./02_SECURE_BOOT.md). This is a requirement, as while it is
...
> decryption password, you can require a PIN to be entered, without which TPM
> won't release the decryption password. This will be explained later.
>
...
- **PCR12:** Overridden kernel command line, credentials (not as important with
  secure boot + UKIs, as the cmdline cannot actually be overridden)
...
We also specify `--tpm2-pcrs=0+7`, which selects the PCR registers that we
decided on above.
