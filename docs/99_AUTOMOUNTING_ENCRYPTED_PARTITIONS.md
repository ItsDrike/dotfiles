...
> [!TIP]
> You can safely skip this guide if you don't use encryption, or have only a
> single encrypted partition, being the one with your root, which systemd
> initrd is already decrypting and mounting for you during boot.
...
> their lifespan. Don't add it if you're using an HDD.
...
LUKS encryption has support for having multiple keys for the same partition.
